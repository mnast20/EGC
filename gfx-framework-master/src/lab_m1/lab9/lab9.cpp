...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, mapTextures["earth"], mapTextures["crate"]);

        // RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, mapTextures["crate"]);
    }
...

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, mapTextures["random"], mapTextures["random"]);
    }
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    glUniform1i(yes_earth, earth);

    int time = glGetUniformLocation(shader->program, "time");
    glUniform1f(time, Engine::GetElapsedTime());

    int yes_grass = glGetUniformLocation(shader->program, "yes_grass");
    glUniform1i(yes_grass, grass);
...
void Lab9::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_M) {
        switch (wrappingMode) {
        case GL_REPEAT:
            magFilter = GL_MIRRORED_REPEAT;
            break;
        case GL_MIRRORED_REPEAT:
            magFilter = GL_CLAMP_TO_EDGE;
            break;
        case GL_CLAMP_TO_EDGE:
            magFilter = GL_CLAMP_TO_BORDER;
            break;
